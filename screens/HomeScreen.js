...
class FormModal extends Component {
  ...
  async submitBilan(){
    if(!this.state.name || !this.state.description){
      alert("Veuillez renseigner un nom à votre bilan")
      return
    }
    ...
    await fetch('http://192.168.8.103:33333/query', {
      ...
      body: JSON.stringify({
        query: "INSERT INTO `bilan` (id, nom, description, date_bilan, img) values ("+Math.floor(Math.random() * 10000)+", '"+this.state.name+"', '"+this.state.description+"', '"+this.formatDate(new Date())+"', '"+this.state.image_data+"');",
      })
    })
        ...
        <Modal
          ...
          <ScrollView>
            <View style={{ marginTop: 22, marginbottom: 30, padding: 20, justifyContent: 'center' }}>
              ...
              />
              <Text style={{ marginTop: 10 }}>Nom du bilan:</Text>
              <TextInput
                style={{ height: 40, borderColor: 'gray', borderBottomWidth: 1, padding: 5, marginBottom: 10 }}
                placeholder="Hangar numero 3"
                onChangeText={(text) => this.setState({name: text})}
                value={this.state.name}
              />
              <Text style={{ marginTop: 10 }}>Description:</Text>
              <TextInput
                style={{ height: 150, borderColor: 'gray', borderBottomWidth: 1, padding: 5, marginBottom: 10 }}
                placeholder="Ce bilan presente les 343 engins du hangar numero 3"
                numberOfLines={4}
                multiline
              ...
              />
              <Button
                title="Enregistrer bilan"
                onPress={() => { this.submitBilan() }}
              />
            </View>
          </ScrollView>
        </Modal>
...
function HomeScreen(props) {
  ...
  takePicture = async () => {
    if (camera) {
      ...
      let response = await fetch('http://192.168.8.103:33333/predict', {
        method: 'POST',
        headers: { 'Content-Type': 'multipart/form-data', 'Accept': 'application/json' },
        body: formdata
      }).then(response => response.json())
      
      let desc = "Cette image comporte:\r\n"
  ...
  // {loading && <View style={{ flex:1 }}><ActivityIndicator size="large" color="#0000ff" /></View>}
  return (
